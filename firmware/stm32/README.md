...
2026-09-10 的两电机三点标定已把 PPR 确认为 `1` 并更新集中配置。Hall 超时和最大
RPM 仍保持 `TBD`/无效，因此当前构建仍会明确输出 `INVALID_CONFIG` 和零有效 RPM，
不会用不完整配置产生看似有效的转速。两项边界完成标定后，同一周期任务路径才会
产生 `VALID`、`TIMED_OUT` 或 `IMPLAUSIBLE_PULSE` 结果。闭环继续默认关闭。
