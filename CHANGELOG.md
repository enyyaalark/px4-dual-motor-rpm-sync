...
- 2026-09-04：Issue #6 双路 Hall 捕获 L1 实机完成——PR #65（双路 L1 串扰证据）合并（`e7b0802`）；捕获固件经 ST-LINK 三线 SWD 下载并校验；PA0/PA1 经 UART 记录无脉冲、Hall 1、Hall 2 与复位后双路捕获，两路 `chN_valid` 均置位、周期非零。`chN_valid` 是历史捕获标志，`chN_age_ms` 变化才表示新脉冲；手动脉冲周期不作为 RPM/PPR 证据。原始记录见 `data/raw/2026-09-04/`，逻辑分析仪周期对照、PPR 与最高预期频率仍待验证。
