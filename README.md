...
- Pixhawk 6C Mini 飞控（硬件修订和 PX4 固件版本 `TBD`）、FlySky FS-iA10B 接收机（实际输出模式 `TBD`）
...
- 飞控外壳型号和接收机型号已由照片确认；Pixhawk 6C Mini 硬件修订/PX4 固件、接收机实际输出模式、第二只电调一致性、电池、螺旋桨和 STM32 引脚仍未确认。
