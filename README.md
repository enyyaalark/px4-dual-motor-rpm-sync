...
**当前状态：控制链硬件链路基本打通。双路 Hall（PA0/PA1 + TIM2）已实机验证；双路 PX4 PWM 输入已实机 `VALID`（MAIN1 捕获已从 PA6 改为 PB4/TIM3_CH1，MAIN2 为 PB6/TIM4_CH1）；TIM1 双路 PWM 输出（PA8/PA10）已通过逻辑分析仪验证；HCT157 旁路默认 A 与 3.3V 高选 B 均 L1 通过；ESC1/ESC2 无桨功能验证完成。闭环 `system_controller` 已接入目标固件，baseline/sync 两版产物已构建，但 Hall RPM 与 P 同步效果尚待安装磁体后实机验证；不具备安全飞行条件。**
...
STM32 应用层使用 C++17，状态机至少包括 `INIT`、`MONITOR_ONLY`、`SYNC_CONTROL`、`BYPASS`、`FAULT`。参数集中在 `app_config.hpp`。目标工程已配置 `PA0/TIM2_CH1`、`PA1/TIM2_CH2` 双路 Hall 捕获、`PB4/TIM3_CH1` 与 `PB6/TIM4_CH1` 双路 PX4 PWM 输入，以及 `PA8/TIM1_CH1`、`PA10/TIM1_CH3` 双路 PWM 输出；闭环 `system_controller` 已接入。CubeMX 生成的 HAL C 代码通过薄适配层接入，HAL 回调只采集固定大小数据。

## 快速开始

文档和工具可立即使用；Issue #3 的最小点灯/UART 工程及 Issue #6 的双路 Hall 捕获工程均可构建。捕获工程已通过 SWD 写入并完成 L1 UART 采集，但仍不能作为最终控制器配置：
...
- [x] 双路 PX4 PWM 输入实机 `VALID`（PB4/MAIN1、PB6/MAIN2）
- [x] TIM1 双路 PWM 输出逻辑分析仪验证
- [x] HCT157 旁路默认 A 与高选 B L1 验证
- [x] ESC1/ESC2 无桨功能验证
- [x] 闭环控制目标固件集成与 baseline/sync 产物构建（待磁体实机验证）
- [ ] 最高脉冲频率与 RPM 精度验收（PPR=1；v2 双路停止归零已观察通过，精确 100 ms 时刻仍待高采样率证据）
- [ ] 硬件型号、电压、协议和引脚确认
- [ ] Hall RPM 实机验证与完整 11 字段 UART CSV 遥测
- [ ] 开环基线、同步控制和故障注入测试
- [ ] 带桨台架数据、演示视频和最终报告

## 已知限制

- 飞控外壳型号、PX4 1.17.0 精确提交、接收机型号（FS-SR8；旧照片识读 FS-iA10B 已更正）、双发后推布局、两只相同电调以及电池额定信息已确认；当前 `SYS_AUTOSTART=2100`、`CA_ROTOR_COUNT=2`，但几何与最终飞行配置仍未验收。Pixhawk 硬件修订、PWM 实际电平、接收机实际输出模式、电池化学体系/实测电压和螺旋桨仍未确认；STM32 PWM 输入双路已实机 `VALID`。
