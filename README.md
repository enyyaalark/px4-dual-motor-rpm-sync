...
本项目面向双发固定翼台架原型：PX4 负责遥控输入、姿态与上层飞行控制；STM32G431 独立测量两台无刷电机的实际转速，在两个基础指令相同的前提下实施有限幅的同步修正，并通过 UART 输出可复核数据。
...
- Pixhawk 6C Mini 飞控（PX4 v1.12.3；硬件修订 `TBD`）、FlySky FS-iA10B 接收机（实际输出模式 `TBD`）
- STM32G431CBU6 WeAct 核心板 ×2、ST-LINK V2、USB 转 TTL
- RS2205 2300KV 无刷电机 ×2、同型号电调 ×2（照片确认其中一只为 Flycolor Raptor5 G071-35A，另一只待复核）
- A3144E ×5（2 使用、3 备用）
...
- 飞控外壳型号、PX4 v1.12.3 和接收机型号已确认；当前 `SYS_AUTOSTART=1001`（HIL Quadcopter X）与双发固定翼目标不一致。Pixhawk 6C Mini 硬件修订、接收机实际输出模式、第二只电调一致性、电池、螺旋桨和 STM32 引脚仍未确认。
