...
**当前状态：规划、代码骨架、STM32 最小 bring-up，以及 PA0/PA1 + TIM2 的双路 Hall 捕获均已实机完成，Issue #6 已按修订范围关闭。Issue #7 已把捕获快照接入 C++17 RPM 评估与 v2 UART 遥测；Issue #8 的 PPR 离线标定工具和 Issue #10 的双路 PWM C/HAL 输出校验边界也已合并。PPR、超时、最大 RPM、ESC 刷新频率和 PWM 范围仍保持无效/TBD，TIM1 未配置或启动；PPR/RPM、PWM、旁路和电机实机验证仍未完成，不具备安全飞行条件。**
