...
- RS2205 2300KV 无刷电机 ×2、Flycolor Raptor5 G071-35A 同型号电调 ×2（用户确认两只一致）
- 3S1P 电池（用户提供额定 11.1V、100C；容量、化学体系、满充电压和连接器 `TBD`）
- A3144E ×5（2 使用、3 备用）
...
- 飞控外壳型号、PX4 v1.12.3、接收机型号、双发后推布局、两只相同电调以及 3S1P/11.1V/100C 电池额定信息已确认；当前 `SYS_AUTOSTART=1001`（HIL Quadcopter X）与目标不一致。Pixhawk 6C Mini 硬件修订、接收机实际输出模式、电池容量/实测电压、螺旋桨和 STM32 引脚仍未确认。
