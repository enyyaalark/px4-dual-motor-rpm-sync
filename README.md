...
**当前状态：规划与代码骨架。尚未完成引脚确认、电气验证、电机测试或飞行验证，不具备安全飞行条件。**
...
- PX4 固件飞控板、遥控器/接收机（型号和协议 `TBD`）
- STM32G431CBU6 WeAct 核心板 ×2、ST-LINK V2、USB 转 TTL
- RS2205 2300KV 无刷电机 ×2、同型号电调 ×2（型号 `TBD`）
- A3144E ×5（2 使用、3 备用）
...
文档和工具可立即使用；固件工程需要等引脚表确认后再由 STM32CubeMX 生成：
...
工作日安排见 [项目计划](docs/project-plan.md)。周六、周日不安排任务。
...
- PX4 型号/固件、接收协议、电调、电池、螺旋桨和 STM32 引脚均未确认。
