...
| Q07 | Hall 捕获 `PA0/PA1 + TIM2_CH1/CH2` 已实机验证；PWM 输入 `PB4/TIM3_CH1 + PB6/TIM4_CH1` 已实机双路采集；TIM1 双路 PWM 输出 `PA8/TIM1_CH1 + PA10/TIM1_CH3` 已通过逻辑分析仪实机验证；旁路选择 `PB2/GPIO` 已配置为初始低电平、仅逻辑分析仪测试入口可切换。HCT157 旁路及最终整机资源仍待验证 | 决定完整 CubeMX 工程与布线 | role:firmware | HCT157 旁路与整机集成 |
...
- Q07/Q12：成员 B 于 2026-09-04 经 ST-LINK 三线 SWD 下载并校验双路捕获固件，随后通过 PA9 单向 UART 记录无脉冲、Hall 1、Hall 2 与复位后双路捕获，两路 `chN_valid` 均置位且周期非零。原始记录见 `data/raw/2026-09-04/`。项目负责人随后取消单独的手动周期对照并按修订范围关闭 Issue #6；PPR 标定、最高预期频率与数字滤波结论转交 Issue #8。
