...
> 建立日期：2026-08-31
> 状态：A3144E→HC14 双路 L1 手动触发、串扰与线束稳定性验证已完成（独立第二颗 SN74HC14N 1A/1Y；限于 20 kHz 采样率、50 µs 时间分辨率与现有捕获窗口；1nF 保留为起始值）；STM32 定时器捕获、RPM 与最高预期脉冲频率验收仍未完成
...
| HC14 -> STM32 霍尔捕获 | SN74HC14N #1 `1Y` / 独立 #2 `1Y` | 3.3V 整形方波（反相） | STM32 TIM2 输入捕获候选 `PA0/PA1` | 定时器输入捕获能力 | 无磁体 0V，触发时约 3.2V；两路接线静态电平通过 | 输出端与接线已验证，STM32 捕获待固件烧录后实机验证 | 2026-09-02、2026-09-03 波形与原始采集（`data/raw/2026-09-02/`、`data/raw/2026-09-03/`，含 `.sr`） |
...
- 2026-09-03 双路证据只覆盖逻辑信号链路与串扰，不证明 STM32 TIM2 捕获、RPM 计算、最终安装距离或电机运行；STM32 捕获待固件烧录与 UART 遥测验证。
