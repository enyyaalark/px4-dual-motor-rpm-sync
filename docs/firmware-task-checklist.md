...
固件进度超前：计划 09-01 完成 #3 点灯/UART，#18 原计划 09-15，但已在 09-01 合并；#31/#33 也已提前完成编码并提交 PR。2026-09-04 进一步完成双路 Hall 捕获固件 SWD 写入与 UART 采集验证。

项目总体尚不能称为超前：M1 所需的 PPR 标定、逻辑分析仪周期对照和 RPM 实机精度证据仍未具备，需等待硬件侧输入。
...
- [ ] Issue #6：双路定时器输入捕获——L1 UART 采集（无脉冲/单路/复位双路）已通过；逻辑分析仪周期对照、最高预期频率与 PPR 仍待验证。
