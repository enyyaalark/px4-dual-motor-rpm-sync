...
固件进度超前：计划 09-01 完成 #3 点灯/UART，#18 原计划 09-15，但已在 09-01 合并；#31/#33 也已提前完成。2026-09-04 进一步完成双路 Hall 捕获固件 SWD 写入与 UART 采集验证，并合并 #7 捕获到 RPM 评估、#8 PPR 离线标定工具及 #10 双路 PWM C/HAL 校验边界。标定参数继续保持无效，TIM1 未配置或启动。
...
- [x] Issue #8 软件切片：PPR/RPM 离线标定分析工具。
  - PR #67 已合并（`8f0bfdb`）；父 Issue 保持 Open + blocked，未生成实机 PPR 或 3% 精度结论。
...
- [ ] Issue #7：双路捕获到 RPM 评估的软件接入、异常状态和停止归零路径已合并；等待 Issue #8 标定 PPR/超时/最大 RPM 后刷写并做实机验收。
