...
固件进度超前：计划 09-01 完成 #3 点灯/UART，#18 原计划 09-15，但已在 09-01 合并；#31/#33 也已提前完成编码并提交 PR。

项目总体尚不能称为超前：M1 所需的霍尔波形、双路定时器捕获、PPR 标定和 RPM 实机证据仍未具备，需等待硬件侧输入。
...
- [x] Issue #31：硬件无关 RPM 计算逻辑（PR #32）。
- [x] Issue #33：硬件无关 PWM 输入逻辑（PR #34）。

## 当前可以继续的小项

- [ ] 检查 PR #32/#34 的 Review 意见并按要求修改。
