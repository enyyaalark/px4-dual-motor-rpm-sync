...
Issue #6 已形成一套无复用冲突的固件候选方案，但成员 A 尚未复核两侧排针可及性和第二路 HC14 布置。在该复核及 CubeMX 冲突检查完成前，不创建最终控制器 `.ioc`，表中候选项不得视为可接线结论。

| 功能 | 方向 | 外部信号 | STM32 定时器/GPIO | 电气要求 | 状态 |
|---|---|---|---|---|---|
| Hall 1 capture | 输入 | HC14 OUT1 | 候选 `PA0 / AF1 / TIM2_CH1` | 3.3 V 整形输出；上升沿捕获 | 待排针与 CubeMX 复核 |
| Hall 2 capture | 输入 | HC14 OUT2 | 候选 `PA1 / AF1 / TIM2_CH2` | 3.3 V 整形输出；上升沿捕获 | 待排针与 CubeMX 复核 |
...
|---|---|---|---|---|
| `TIM2` | Hall 1/2 上升沿捕获 | 候选 1 MHz 自由运行、32 位递增计数 | 两路共享同一时间基准；1 µs/tick 时约 71.6 分钟回绕，应用层按无符号 32 位差值处理单次回绕 | 时钟树、输入滤波、极性、最大脉冲频率和中断负载 |
