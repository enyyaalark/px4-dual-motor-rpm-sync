...
SN74HC14 OUT --------------------------- STM32 timer input（见下方实际映射）
```

Issue #6 当前 L1 实际接法使用两颗独立 SN74HC14N 的第一组门：第一颗 `1Y -> PA0/TIM2_CH1`，第二颗 `1Y -> PA1/TIM2_CH2`；两颗芯片的 `1A` 分别接各自 RC 输出。不要按旧交接使用第二颗的 `2A/2Y`。这只确认物理连接关系，不代表双路捕获波形已经验收。
