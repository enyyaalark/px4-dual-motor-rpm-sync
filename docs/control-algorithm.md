...
PPR 已于 2026-09-10 标定为 1；停止超时和最大合理 RPM 尚未标定，继续保持
`TBD`/零值。任一必需参数为零值、负值、NaN 或无穷值都属于无效配置，不能输出
有效 RPM。计算结果分为：
...
Issue #7 已通过 C/C++ 薄适配层把这些状态接入双路 HAL 捕获快照和版本化 UART 遥测，但尚未接入最终故障标志或控制状态机。最大 RPM、脉冲跳变阈值和停止超时必须根据实测波形及转速范围确定，主机合成测试不能代替该标定。
