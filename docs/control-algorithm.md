...
`pwm_input` 的纯逻辑接口由调用方提供最小脉宽、最大脉宽和超时。2026-09-11 已测得 PX4 MAIN1/2 为 400 Hz，静态 1000 µs、测试指令 1300 µs，配置端点为 disarmed 1000 µs、active 1100–1900 µs。因此 `app_config.hpp` 采用 950–1950 µs 监测窗口和 10 ms 超时（约四个丢失帧）作为 `MONITOR_ONLY` 实机验证初值。它不启用闭环，也不替代 STM32 实机捕获、PWM 电平和 ESC 范围验证。状态包括：
...
毫秒时间使用无符号 32 位差值，可处理两次检查间最多一次回绕。主机测试证明状态语义，不证明 STM32 捕获精度或信号电平；若实机结果不满足当前窗口或超时，必须根据原始证据重新评审参数。
...
- 任一路请求为 NaN 或无穷值时返回 `kInvalidRequest`，整组两路输出均不可用，避免只更新一侧；
- 返回状态不是 `kValid` 时，两路有效脉宽保持 0。未来 HAL 适配层必须检查状态并保持定时器输出未启用，不能把 0 当作已验证的 ESC 指令。

该接口只验证数值边界，不产生波形。输出频率、计数器换算、同步更新、启动/复位瞬态和真实限幅仍需逻辑分析仪完成 T07。
...
`hall_monitor` 的纯逻辑接口由调用方传入计时器频率、PPR、停止超时和最大合理 RPM。
PPR 已于 2026-09-10 标定为 1；团队同时接受 `100 ms` 停止超时和 `3300 RPM` 最大
合理值作为 `MONITOR_ONLY` 复核初值。100 ms 来自 34.787 ms 最长入选周期，3300 RPM
是在 3000 RPM 硬件测试安全上限之上的 10% 异常判定余量；两者尚未实机验证，且
3300 RPM 不提高台架许可转速。任一必需参数为零值、负值、NaN 或无穷值都属于无效
配置，不能输出有效 RPM。计算结果分为：
...
- 最终 PWM 限幅：软件测试使用 1000–2000µs synthetic 向量；项目默认值保持无效，必须用实际电调校准替换后才可启用。
- RPM 死区和低速闭环禁止。
