...
| 飞控 | Pixhawk 6C Mini | 1，照片与 USB/NSH 确认 | PX4 1.17.0，提交 `d6f12ad1c4f70ad3230afd7d86e971421e02fef4`；制造商/硬件修订仍为 `TBD`；双发几何与最终飞行配置未验收 |
...
|---|---|---|---|
| PX4 -> STM32 | 台架映射为 MAIN1=左侧、MAIN2=右侧；400 Hz，disarmed 1000 µs、active 1100–1900 µs；1000/1300 µs 波形已测 | 3.3V 容限和定时器捕获 | 逻辑与时序已确认；输出电压仍待测，不得仅凭参数接线 |
...
## 2026-09-11 PX4 1.17.0 与 PWM 输入基线

NSH 只读结果确认当前飞控为 FMUv6C、PX4 1.17.0（提交 `d6f12ad1c4f70ad3230afd7d86e971421e02fef4`），`SYS_AUTOSTART=2100`、`SYS_HITL=0`、`SYS_AUTOCONFIG=0`。MAIN1/2 分别配置为 Motor 1/2（功能 101/102），400 Hz，disarmed 1000 µs、active 1100–1900 µs；`CA_ROTOR_COUNT=2`，但转子几何尚未完成飞行配置验收。

成员 A 在仅 USB 供电、无动力输出条件下用逻辑分析仪确认两路静态信号均约为 400 Hz、1000 µs，误差约 ±1 µs。随后分别执行有界 `actuator_test`，观察到 Motor 2 只改变 MAIN2、Motor 1 只改变 MAIN1，目标脉宽为 1300 µs，约 10 秒后均返回 1000 µs；约 500 ms 的变化斜坡符合当前 PX4 测试输出实现。上述证据支持 STM32 `MONITOR_ONLY` 输入捕获测试，但不确认 PWM 电压，也不授权连接 ESC、电机或电池。
